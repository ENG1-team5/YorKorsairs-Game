...
public class Particle {
    ...
    Particle(String path, Vector2 pos_, float size_, float speed_, float time_) { this(path, pos_, size_, size_, speed_, time_); }
    Particle(String path, Vector2 pos_, float sizeStart_, float sizeEnd_, float speed_, float time_) {
        this(path, pos_, sizeStart_, sizeEnd_, new Vector2((float)Math.random()-0.5f, (float)Math.random()-0.5f).nor().scl(speed_), time_); }
    Particle(String path, Vector2 pos_, float size_, Vector2 vel_, float time_) { this(path, pos_, size_, size_, vel_, time_); }
